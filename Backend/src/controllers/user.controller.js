...
const registerUser = asyncHandler(async (req, res) => {
  ...
  //check if avatar has been uploaded on cloudinary or not
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  // send secure cookie and success response
  ...
  //updated refreshToken in existing user or query the db for updated user.
  const loggedInUser = await User.findById(user._id).select(
    "-password -refreshToken"
  );

  //securing the cookies so that they can only be modified by the server
  const options = {
    httpOnly: true,
    secure: true,
  };
...
const getUserChannelProfile = asyncHandler(async(req,res)=>{
  ...
  if(!username?.trim()){
    throw new ApiError(400,"username is missing while finding channels");
  }
  ...
  // console.log(channel) //to see what value is returned by aggregate
  if(!channel?.length){
